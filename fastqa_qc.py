...
def check_extention(fastf):
    extention = ""
    if fastf.endswith(".fasta") or fastf.endswith(".fa") or fastf.endswith(".fas") or fastf.endswith(".fna"):
        extention = "fasta"
...
def assembly_stat(input_file, exten):
    ...
    print("GC ratio {}%".format(round(100 * (g_count + c_count) / total_length), 1))
